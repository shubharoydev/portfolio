...
const BackgroundBeamsWithCollision = ({ children, className }) => {
  ...
  return (
    <div
      ref={parentRef}
      className={cn(
        "min-h-screen bg-gradient-to-b from-[#0C1324] to-black relative w-full overflow-hidden",
        "before:content-[''] before:absolute before:inset-0 before:bg-[radial-gradient(circle_at_center,rgba(255,255,255,0.05)_1px,transparent_1px)] before:bg-[length:10px_10px]",
        className
      )}
    >
      {beams.map((beam) => (
      ...
      <div
        ref={containerRef}
        className="absolute bottom-0 bg-blue-900 w-full inset-x-0 pointer-events-none"
        style={{
          boxShadow:
